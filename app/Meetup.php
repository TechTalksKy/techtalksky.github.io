...
class Meetup 
{
   ...
   */        
    ...
    if(in_array('key', $params) && $url!=self::ACCESS && $url!=self::AUTHORIZE)
        ...
        if (!is_null($this->_response && $status == 201)) {
            return $this->_response;
        } elseif (!is_null($this->_response) && $status != 200) {
    ...
    public static function createEvent($submission, $date)
    {
        $token = json_decode(Token::where('provider', 'meetup')->first()->token);
        
        $client = new self([
            'access_token' => $token->token
        ]);

        $venue_id = ($submission->city == 'Lexington') ? 24826390 : 24219929;
        // The time value is a bit of a temp fix, need to figure out more reliable method
        return $client->post('/2/event', [
             'group_id' => 20581827,
             'group_urlname' => 'Tech-Talks-KY',
             'name' => $submission->title,
             'time' => $date->addHour(5)->timestamp * 1000,
